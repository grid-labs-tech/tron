...
def ensure_cluster_instance(
    repository: Any, component: ApplicationComponentModel, cluster: Any
...
def get_or_create_cluster_instance(
    repository: Any, db: Session, component: ApplicationComponentModel
) -> ClusterInstanceModel:
    """
...
def handle_enabled_change(
    ...
    if enabled_changed["was_enabled"] and not enabled_changed["will_be_enabled"]:
        ...
        deploy_to_k8s_func(
            component, component.instance.environment_id, cluster, cluster_instance
        )
...
def deploy_to_kubernetes(
    ...
    try:
        ...
        db.rollback()
        raise Exception(
            f"Failed to deploy {component_type} to Kubernetes cluster '{cluster.name}': {str(e)}"
        )
...
def delete_from_kubernetes_safe(
    ...
    """
    try:
        settings = repository.find_settings_by_environment_id(
            component.instance.environment_id
        )
        settings_serialized = serialize_settings(settings)
        ...
        print(
            f"Error removing {component_type} '{component.name}' from Kubernetes: {e}"
        )
        db.commit()
        db.refresh(component)
...
def update_component_enabled_field(
    component: ApplicationComponentModel, enabled: bool, repository: Any
) -> Dict[str, Any]:
    """
